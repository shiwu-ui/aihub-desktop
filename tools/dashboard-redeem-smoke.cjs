...
async function run() {
  ...
  try {
    ...
    await page.evaluate(async () => {
      ...
      window.__dashboardFailures = { stats: false, snapshot: false, announcements: false }
      request = async (route) => {
        if (route === '/auth/me') return user
        if (route === '/usage/dashboard/stats') {
          if (window.__dashboardFailures.stats) throw new Error('dashboard stats unavailable')
          return { active_api_keys: 29, total_api_keys: 29, today_requests: 1696, total_requests: 41165, today_actual_cost: 6.6977, total_actual_cost: 93.7206, today_tokens: 272800000, today_input_tokens: 18400000, today_output_tokens: 619900, total_tokens: 1164900000, total_input_tokens: 391700000, total_output_tokens: 7300000, rpm: 1, tpm: 108300, average_duration_ms: 6760, by_platform: [{ platform: 'openai', total_actual_cost: 8, today_actual_cost: 2, total_requests: 20, total_tokens: 2000 }] }
        }
        if (route.startsWith('/usage/dashboard/snapshot-v2')) {
          if (window.__dashboardFailures.snapshot) throw new Error('dashboard snapshot unavailable')
          return { start_date: '2026-07-01', end_date: '2026-07-19', trend: [] }
        }
        ...
        if (route === '/announcements') {
          if (window.__dashboardFailures.announcements) throw new Error('announcements unavailable')
    ...
    await page.evaluate(async () => {
      window.__dashboardFailures = { stats: true, snapshot: true, announcements: true }
      await navigate('dashboard')
    })
    await page.waitForSelector('.dashboard-quick-actions')
    const partialDashboard = await page.textContent('#content')
    assert.ok(partialDashboard.includes('$26.28'))
    assert.equal(await page.locator('.dashboard-stats-error').count(), 1)
    assert.equal(await page.locator('.dashboard-snapshot-error').count(), 1)
    assert.equal(await page.locator('.dashboard-announcements-error').count(), 1)
    ...
    assert.doesNotMatch(await page.textContent('#content'), /套餐|订阅/)
